...
/**
 * 要实现从数据库中获取内容，不过这边先尝试案例
 * @author Pyrojewel
 * */
public class SubjectRepertory {

    public static List<CourseModel> loadDefaultSubjects(){
        //json转义
    ...
    public static List<CourseModel> loadDefaultSubjects1(ModelOpenHelper modelOpenHelper){
        System.out.println("111"+modelOpenHelper.getCourseModelQueryByName("高等数学A(下)"));
        return modelOpenHelper.getAllCourseConModels();
    }
    public static List<CourseModel> loadDefaultSubjects2() {
        //json转义
     ...
     * @return
     */
    public static List<CourseModel> parse(String parseString) {
        ...
        try {
            ...
            for(int i=0;i<array.length();i++){
                ...
                if(array2.length()<=10){
                    courses.add(new CourseModel(term,name,null, teacher, null, -1, -1, -1, -1,null,5));
                    continue;
                }
                ...
                courses.add(new CourseModel(term,name, room, teacher, getWeekList(weeks), start, step, day, -1,null,5));
            }
